...
int SUDOKU::Backtrack(Write_File* w_obj, int t, bool w, bool distinct) {
    if (t < 0 || t > DIM * DIM) {
    ...
    if (t == DIM * DIM) {
        if (w)
            Display(w_obj, "Final board: ");
        return 1;
    }
    int x = t / DIM, y = t % DIM;
    // Get to the next one
    if (board[x][y]) {
        return Backtrack(w_obj, t + 1, w, distinct);
    } else {
        // Empty point. Test for all the values
        for (int i = 1; i < DIM + 1; i++) {
            board[x][y] = i;
            if (CorrectPlace(x, y)) {
                result_num += Backtrack(w_obj, t + 1, w, distinct);
...
bool SUDOKU::StartGen(int num, Write_File* w, int b_min, int b_max, bool d) {
    if (num <= 0 || num > 1000000) return false;
    if (b_min < 1 || b_max > 64 || b_max < b_min) {
        return false;
    }
    int cur_pos;
    int boards_number = 0;
    ...
    int index[DIM] = { 0, 1, 2, 3, 4, 5, 6, 7, 8 };
    /*srand((unsigned)time(NULL));*/
    while (boards_number < num) {
        int blanks = b_min + rand() % (b_max - b_min + 1);

        // Get the random first row
        string first_line = gen_first_line();
        for (int i = 0; i < DIM; i++) {
            w->board[0][i] = first_line[i];
        }
        ...
        for (int line = 1; line < DIM; line++) {
            ...
            for (int j = 0; j < DIM; j++, cur_pos++) {
                cur_pos = (cur_pos == DIM) ? 0 : cur_pos;
                w->board[line][j] = w->board[0][index[cur_pos]];
            }
        }
        ...
        if (d) {
            int result_num = Backtrack(w, 0, 0, d);
            if (result_num > 1) {
                continue;
            }
        }
